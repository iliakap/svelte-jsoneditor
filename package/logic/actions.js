import { createInsideSelection, createKeySelection, createMultiSelection, createValueSelection, hasSelectionContents, isEditingSelection, isKeySelection, isValueSelection, selectionToPartialJson } from './selection';
import copyToClipboard from '../utils/copyToClipboard';
import { createNewValue, createRemoveOperations, insert } from './operations';
...
import { expandAll, expandPath, expandRecursive, expandWithCallback } from './documentState';
import { initial, isEmpty, last } from 'lodash-es';
import { insertActiveElementContents } from '../utils/domUtils';
...
// TODO: write unit tests
export function onPaste({ clipboardText, json, documentState, readOnly, parser, onPatch, onChangeText, openRepairModal }) {
    if (readOnly) {
        return;
    }
...
// TODO: write unit tests
export function onInsert({ insertType, selectInside, refJsonEditor, json, documentState, readOnly, parser, onPatch, onReplaceJson }) {
    if (readOnly || !documentState.selection) {
        return;
    }
...
// TODO: write unit tests
export async function onInsertCharacter({ char, selectInside, refJsonEditor, json, documentState, readOnly, parser, onPatch, onReplaceJson, onSelect }) {
...
async function onInsertValueWithCharacter({ char, refJsonEditor, json, documentState, readOnly, parser, onPatch, onReplaceJson }) {
    if (readOnly || !documentState.selection) {
        return;
    }
